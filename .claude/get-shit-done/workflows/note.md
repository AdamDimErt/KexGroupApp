...
- **Global scope**: `D:/kexgroupapp/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/kexgroupapp/.claude/notes/`)
...
2. Glob `D:/kexgroupapp/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/kexgroupapp/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/kexgroupapp/.claude/notes/` — works in any directory
